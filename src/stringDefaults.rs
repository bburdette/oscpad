...
};Elm.NoDragEvents = Elm.NoDragEvents || {};
Elm.NoDragEvents.make = function (_elm) {
   "use strict";
   _elm.NoDragEvents = _elm.NoDragEvents || {};
   if (_elm.NoDragEvents.values) return _elm.NoDragEvents.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   ...
   $VirtualDom = Elm.VirtualDom.make(_elm);
   var _op = {};
   var on = F3(function (name,deco,ftn) {
      return A4($VirtualDom.onWithOptions,
      name,
      A2($VirtualDom.Options,true,true),
      deco,
      ftn);
   });
   ...
   return _elm.NoDragEvents.values = {_op: _op
                                     ,onBegin: onBegin
                                     ,onEnd: onEnd
                                     ,onRepeat: onRepeat
                                     ,onAbort: onAbort
...
Elm.Template.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var renderComponent = F3(function (record,component,result) {
      var _p0 = component;
      if (_p0.ctor === "Literal") {
   ...
   var render = F2(function (template,record) {
      return A3($List.foldr,renderComponent(record),"",template);
   });
   var Interpolation = function (a) {
      return {ctor: "Interpolation",_0: a};
   };
   var withValue = F2(function (interpolator,template) {
      return A2($List._op["::"],
      Interpolation(interpolator),
      template);
   });
   var andValue = $Basics.flip(withValue);
   var Literal = function (a) {
      return {ctor: "Literal",_0: a};
   };
   var template = function (initial) {
      return _U.list([Literal(initial)]);
   };
   var withString = F2(function (string,template) {
      return A2($List._op["::"],Literal(string),template);
   });
   var andString = $Basics.flip(withString);
...
Elm.Template.Infix = Elm.Template.Infix || {};
Elm.Template.Infix.make = function (_elm) {
   "use strict";
   _elm.Template = _elm.Template || {};
   _elm.Template.Infix = _elm.Template.Infix || {};
   if (_elm.Template.Infix.values)
   return _elm.Template.Infix.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   _op["%>"] = $Template.andString;
   _op["<%"] = $Template.andValue;
   return _elm.Template.Infix.values = {_op: _op};
...
Elm.SvgTextSize = Elm.SvgTextSize || {};
Elm.SvgTextSize.make = function (_elm) {
   "use strict";
   _elm.SvgTextSize = _elm.SvgTextSize || {};
   if (_elm.SvgTextSize.values) return _elm.SvgTextSize.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$SvgTextSize = Elm.Native.SvgTextSize.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var getTextWidth = F2(function (text,font) {
      return A2($Native$SvgTextSize.getTextWidth,text,font);
   });
   return _elm.SvgTextSize.values = {_op: _op
                                    ,getTextWidth: getTextWidth};
};Elm.SvgThings = Elm.SvgThings || {};
Elm.SvgThings.make = function (_elm) {
   "use strict";
   _elm.SvgThings = _elm.SvgThings || {};
   if (_elm.SvgThings.values) return _elm.SvgThings.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   ...
   $SvgTextSize = Elm.SvgTextSize.make(_elm),
   $Template = Elm.Template.make(_elm),
   $Template$Infix = Elm.Template.Infix.make(_elm);
   var _op = {};
   var computeFontScaling = F4(function (fw,fh,rw,rh) {
      var hr = rh / fh;
      var wr = rw / fw;
      return _U.cmp(wr,hr) < 0 ? wr : hr;
   });
   var calcText = F5(function (fontFam,
   lbtext,
   labelMeasuredWidth,
   fontScaling,
   rect) {
      var tmpl = A2($Template$Infix._op["%>"],
      A2($Template$Infix._op["<%"],
      A2($Template$Infix._op["%>"],
      A2($Template$Infix._op["<%"],
      A2($Template$Infix._op["%>"],
      A2($Template$Infix._op["<%"],
      ...
      ")");
      var yc = $Basics.toFloat(rect.y) + $Basics.toFloat(rect.h) / 2;
      var xc = $Basics.toFloat(rect.x) + $Basics.toFloat(rect.w) / 2;
      var scale = fontScaling;
      var yt = yc + 20.0 * scale * 0.5;
      var width = labelMeasuredWidth;
      var xt = xc - $Basics.toFloat(width) * scale * 0.5;
      var xf = A2($Template.render,
      tmpl,
      {scale: $Basics.toString(scale)
      ,xt: $Basics.toString(xt)
      ,yt: $Basics.toString(yt)});
      return _U.list([A2($Svg.text$,
      _U.list([$Svg$Attributes.fill("black")
              ,$Svg$Attributes.fontSize("20px")
              ,$Svg$Attributes.fontFamily(fontFam)
              ,$Svg$Attributes.transform(xf)]),
      _U.list([$Svg.text(lbtext)]))]);
   });
   var calcTextSvg = F3(function (fontFam,textString,rect) {
      var w = A2($SvgTextSize.getTextWidth,
      textString,
      A2($Basics._op["++"],"20px ",fontFam));
      var fs = A4(computeFontScaling,
      $Basics.toFloat(w),
      20.0,
      $Basics.toFloat(rect.w),
      $Basics.toFloat(rect.h));
      return A5(calcText,fontFam,textString,w,fs,rect);
   });
   var processProps = F2(function (controlcount,lst) {
      var l = $List.length(lst);
      var r = _U.cmp(controlcount,l) > 0 ? controlcount - l : 0;
      var lst = A2($List.append,
      A2($List.take,controlcount,lst),
      A2($List.repeat,r,0.0));
      var s = $List.sum(lst);
      return A2($List.map,function (x) {    return x / s;},lst);
   });
   var somme = F2(function (f,lst) {
      var _p0 = $List.head(lst);
      if (_p0.ctor === "Nothing") {
            return lst;
         } else {
            var tl = $List.tail(lst);
            var s = f + _p0._0;
            var _p1 = tl;
            if (_p1.ctor === "Nothing") {
                  return _U.list([s]);
               } else {
                  return A2($List._op["::"],f,A2(somme,s,_p1._0));
               }
         }
   });
   var SRect = F4(function (a,b,c,d) {
      return {x: a,y: b,w: c,h: d};
   });
   var toSRect = function (rect) {
      return A4(SRect,
      $Basics.toString(rect.x),
      $Basics.toString(rect.y),
      $Basics.toString(rect.w),
      $Basics.toString(rect.h));
   };
   var containsXY = F3(function (rect,x,y) {
      return _U.cmp(rect.x,x) < 1 && (_U.cmp(rect.w,
      x - rect.x) > -1 && (_U.cmp(rect.y,y) < 1 && _U.cmp(rect.h,
      y - rect.y) > -1));
   });
   var Rect = F4(function (a,b,c,d) {
      return {x: a,y: b,w: c,h: d};
   });
   var shrinkRect = F2(function (border,rect) {
      return A4(Rect,
      rect.x + border,
      rect.y + border,
   ...
   var mekhr = F3(function (br,w,i) {
      return A4(Rect,br.x + w * i,br.y,w,br.h);
   });
   var hrects = F2(function (rct,count) {
      var idxs = _U.range(0,count - 1);
      var w = $Basics.round($Basics.toFloat(rct.w) / count);
      return A2($List.map,A2(mekhr,rct,w),idxs);
   });
   var mekhrp = F2(function (prect,_p2) {
      var _p3 = _p2;
      return A4(Rect,_p3._0,prect.y,_p3._1,prect.h);
   });
   var hrectsp = F3(function (rct,count,props) {
      var fw = $Basics.toFloat(rct.w);
      var props = A2(processProps,count,props);
      var widths = A2($List.map,
      function (p) {
         return $Basics.round(p * fw);
      },
      props);
      var xes = A2(somme,rct.x,widths);
      return A2($List.map,
      mekhrp(rct),
      A3($List.map2,
      F2(function (v0,v1) {
   ...
   var mekvrp = F2(function (prect,_p4) {
      var _p5 = _p4;
      return A4(Rect,prect.x,_p5._0,prect.w,_p5._1);
   });
   var vrectsp = F3(function (rct,count,props) {
      var fh = $Basics.toFloat(rct.h);
      var props = A2(processProps,count,props);
      var heights = A2($List.map,
      function (p) {
         return $Basics.round(p * fh);
      },
      props);
      var yes = A2(somme,rct.y,heights);
      return A2($List.map,
      mekvrp(rct),
      A3($List.map2,
      F2(function (v0,v1) {
   ...
   var mekvr = F3(function (br,h,i) {
      return A4(Rect,br.x,br.y + h * i,br.w,h);
   });
   var vrects = F2(function (rct,count) {
      var idxs = _U.range(0,count - 1);
      var h = $Basics.round($Basics.toFloat(rct.h) / count);
      return A2($List.map,A2(mekvr,rct,h),idxs);
   });
   var decodeControlId = $Json$Decode.list($Json$Decode.$int);
   var encodeControlId = function (cid) {
   ...
   var jsOrientation = function (o) {
      var _p6 = o;
      switch (_p6)
      {case "vertical": return $Json$Decode.succeed(Vertical);
         case "horizontal": return $Json$Decode.succeed(Horizontal);
         default: return $Json$Decode.succeed(Horizontal);}
   };
   var ff = "sans-serif";
   return _elm.SvgThings.values = {_op: _op
                                  ,ff: ff
                                  ,Vertical: Vertical
                                  ,Horizontal: Horizontal
                                  ,jsOrientation: jsOrientation
                                  ,encodeControlId: encodeControlId
                                  ,decodeControlId: decodeControlId
                                  ,Rect: Rect
                                  ...
                                  ,calcTextSvg: calcTextSvg
                                  ,calcText: calcText
                                  ,computeFontScaling: computeFontScaling};
};Elm.Native = Elm.Native || {};
Elm.Native.Touch = {};
...
Elm.Touch.make = function (_elm) {
   ...
   return _elm.Touch.values = {_op: _op
                              ...
                              ,taps: taps
                              ,Touch: Touch};
};Elm.SvgButton = Elm.SvgButton || {};
Elm.SvgButton.make = function (_elm) {
   "use strict";
   _elm.SvgButton = _elm.SvgButton || {};
   if (_elm.SvgButton.values) return _elm.SvgButton.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $NoDragEvents = Elm.NoDragEvents.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Svg = Elm.Svg.make(_elm),
   $Svg$Attributes = Elm.Svg.Attributes.make(_elm),
   $SvgThings = Elm.SvgThings.make(_elm),
   $Task = Elm.Task.make(_elm),
   ...
   var resize = F2(function (model,rect) {
      var ts = A3($SvgThings.calcTextSvg,
      $SvgThings.ff,
      model.label,
      rect);
      return {ctor: "_Tuple2"
   ...
   var encodeUpdateType = function (ut) {
      var _p0 = ut;
      if (_p0.ctor === "Press") {
            return $Json$Encode.string("Press");
         } else {
            return $Json$Encode.string("Unpress");
         }
   };
   var encodeUpdateMessage = function (um) {
      return $Json$Encode.object(_U.list([{ctor: "_Tuple2"
                                          ,_0: "controlType"
                                          ,_1: $Json$Encode.string("button")}
                                         ,{ctor: "_Tuple2"
                                          ,_0: "controlId"
                                          ,_1: $SvgThings.encodeControlId(um.controlId)}
                                         ,{ctor: "_Tuple2"
                                          ,_0: "updateType"
                                          ,_1: encodeUpdateType(um.updateType)}]));
   };
   var UpdateMessage = F2(function (a,b) {
      return {controlId: a,updateType: b};
   });
   var Unpress = {ctor: "Unpress"};
   var Press = {ctor: "Press"};
   var jsUpdateType = function (ut) {
      var _p1 = ut;
      switch (_p1)
      {case "Press": return $Json$Decode.succeed(Press);
         case "Unpress": return $Json$Decode.succeed(Unpress);
         default: return $Json$Decode.succeed(Unpress);}
   };
   var jsUpdateMessage = A3($Json$Decode.object2,
   UpdateMessage,
   A2($Json$Decode._op[":="],
   "controlId",
   $SvgThings.decodeControlId),
   A2($Json$Decode.andThen,
   A2($Json$Decode._op[":="],"updateType",$Json$Decode.string),
   jsUpdateType));
   var SvgTouch = function (a) {
      return {ctor: "SvgTouch",_0: a};
   };
   var SvgUpdate = function (a) {
      return {ctor: "SvgUpdate",_0: a};
   };
   var Reply = function (a) {    return {ctor: "Reply",_0: a};};
   var UselessCrap = {ctor: "UselessCrap"};
   var pressup = F2(function (model,ut) {
      var um = A2($Json$Encode.encode,
      0,
      encodeUpdateMessage(A2(UpdateMessage,model.cid,ut)));
      return {ctor: "_Tuple2"
             ,_0: _U.update(model,{pressed: _U.eq(ut,Press)})
             ,_1: $Effects.task(A2($Task.andThen,
             model.sendf(um),
             function (_p2) {
                return $Task.succeed(UselessCrap);
             }))};
   });
   var update = F2(function (action,model) {
      var _p3 = action;
      switch (_p3.ctor)
      {case "SvgPress": return A2(pressup,model,Press);
         case "SvgUnpress": var _p4 = model.pressed;
           if (_p4 === true) {
                 return A2(pressup,model,Unpress);
              } else {
                 return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
              }
         case "UselessCrap": return {ctor: "_Tuple2"
                                    ,_0: model
                                    ,_1: $Effects.none};
         case "Reply": return {ctor: "_Tuple2"
                              ,_0: _U.update(model,{name: _p3._0})
                              ,_1: $Effects.none};
         case "SvgUpdate": var pressedupdate = function () {
              var _p5 = _p3._0.updateType;
              if (_p5.ctor === "Press") {
                    return true;
                 } else {
                    return false;
           ...
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,{pressed: pressedupdate})
                  ,_1: $Effects.none};
         default: return $List.isEmpty(_p3._0) ? _U.eq(model.pressed,
           true) ? A2(pressup,model,Unpress) : {ctor: "_Tuple2"
                                               ,_0: model
                                               ,_1: $Effects.none} : _U.eq(model.pressed,false) ? A2(pressup,
           model,
           Press) : {ctor: "_Tuple2",_0: model,_1: $Effects.none};}
   });
   var SvgUnpress = {ctor: "SvgUnpress"};
   var SvgPress = {ctor: "SvgPress"};
   var buttColor = function (pressed) {
      var _p6 = pressed;
      if (_p6 === true) {
            return "#f000f0";
         } else {
   ...
   var view = F2(function (address,model) {
      return A2($Svg.g,
      _U.list([$NoDragEvents.onMouseDown(A2($Signal.message,
              address,
              SvgPress))
              ,$NoDragEvents.onMouseUp(A2($Signal.message,address,SvgUnpress))
              ,$NoDragEvents.onMouseOut(A2($Signal.message,
              address,
              SvgUnpress))]),
      _U.list([A2($Svg.rect,
              _U.list([$Svg$Attributes.x(model.srect.x)
                      ,$Svg$Attributes.y(model.srect.y)
                      ,$Svg$Attributes.width(model.srect.w)
                      ,$Svg$Attributes.height(model.srect.h)
                      ,$Svg$Attributes.rx("15")
                      ,$Svg$Attributes.ry("15")
                      ,$Svg$Attributes.style(A2($Basics._op["++"],
                      "fill: ",
                      A2($Basics._op["++"],buttColor(model.pressed),";")))]),
              _U.list([]))
              ,A2($Svg.text$,
              _U.list([$Svg$Attributes.fill("white")
                      ,$Svg$Attributes.textAnchor("middle")]),
              _U.list([$Svg.text(model.name)]))]));
   });
   ...
   var init = F4(function (sendf,rect,cid,spec) {
      var ts = function () {
         var _p7 = spec.label;
         if (_p7.ctor === "Just") {
               return A3($SvgThings.calcTextSvg,$SvgThings.ff,_p7._0,rect);
            } else {
               return _U.list([]);
            }
      }();
      return {ctor: "_Tuple2"
             ,_0: A8(Model,
             spec.name,
             A2($Maybe.withDefault,"",spec.label),
             cid,
             rect,
             A4($SvgThings.SRect,
             $Basics.toString(rect.x),
   ...
   var Spec = F2(function (a,b) {    return {name: a,label: b};});
   var jsSpec = A3($Json$Decode.object2,
   Spec,
   A2($Json$Decode._op[":="],"name",$Json$Decode.string),
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "label",
   $Json$Decode.string)));
   return _elm.SvgButton.values = {_op: _op
                                  ,Spec: Spec
                                  ,jsSpec: jsSpec
                                  ,Model: Model
                                  ,init: init
                                  ,buttColor: buttColor
                                  ,SvgPress: SvgPress
                                  ,SvgUnpress: SvgUnpress
                                  ,UselessCrap: UselessCrap
                                  ,Reply: Reply
                                  ,SvgUpdate: SvgUpdate
                                  ,SvgTouch: SvgTouch
                                  ,Press: Press
                                  ...
                                  ,encodeUpdateMessage: encodeUpdateMessage
                                  ,encodeUpdateType: encodeUpdateType
                                  ,jsUpdateMessage: jsUpdateMessage
                                  ,jsUpdateType: jsUpdateType
                                  ,update: update
                                  ,pressup: pressup
                                  ,resize: resize
                                  ,view: view};
};Elm.SvgSlider = Elm.SvgSlider || {};
Elm.SvgSlider.make = function (_elm) {
   "use strict";
   _elm.SvgSlider = _elm.SvgSlider || {};
   if (_elm.SvgSlider.values) return _elm.SvgSlider.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $VirtualDom = Elm.VirtualDom.make(_elm);
   var _op = {};
   var sliderEvt = F3(function (evtname,mkaction,address) {
      return A4($VirtualDom.onWithOptions,
      evtname,
      A2($VirtualDom.Options,true,true),
      $Json$Decode.value,
      function (v) {
         return A2($Signal.message,address,mkaction(v));
      });
   });
   _op["=>"] = F2(function (v0,v1) {
      return {ctor: "_Tuple2",_0: v0,_1: v1};
   ...
   var encodeUpdateType = function (ut) {
      var _p0 = ut;
      switch (_p0.ctor)
      {case "Press": return $Json$Encode.string("Press");
         case "Move": return $Json$Encode.string("Move");
         default: return $Json$Encode.string("Unpress");}
   };
   var encodeUpdateMessage = function (um) {
      return $Json$Encode.object(_U.list([{ctor: "_Tuple2"
                                          ,_0: "controlType"
                                          ,_1: $Json$Encode.string("slider")}
                                         ,{ctor: "_Tuple2"
                                          ,_0: "controlId"
                                          ,_1: $SvgThings.encodeControlId(um.controlId)}
                                         ,{ctor: "_Tuple2"
                                         ...
                                         ,{ctor: "_Tuple2"
                                          ,_0: "location"
                                          ,_1: $Json$Encode.$float(um.location)}]));
   };
   var UpdateMessage = F3(function (a,b,c) {
   ...
   var jsUpdateType = function (ut) {
      ...
      switch (_p1)
      {case "Press": return $Json$Decode.succeed(Press);
         case "Move": return $Json$Decode.succeed(Move);
         case "Unpress": return $Json$Decode.succeed(Unpress);
         default: return $Json$Decode.succeed(Unpress);}
   };
   var jsUpdateMessage = A4($Json$Decode.object3,
   UpdateMessage,
   A2($Json$Decode._op[":="],
   "controlId",
   $SvgThings.decodeControlId),
   ...
   A2($Json$Decode._op[":="],"updateType",$Json$Decode.string),
   jsUpdateType),
   A2($Json$Decode._op[":="],"location",$Json$Decode.$float));
   var getY = A2($Json$Decode._op[":="],
   "clientY",
   $Json$Decode.$int);
   var getX = A2($Json$Decode._op[":="],
   "clientX",
   $Json$Decode.$int);
   var getLocation = F2(function (model,v) {
      var _p2 = model.orientation;
      if (_p2.ctor === "Horizontal") {
            var _p3 = A2($Json$Decode.decodeValue,getX,v);
            if (_p3.ctor === "Ok") {
                  return $Result.Ok($Basics.toFloat(_p3._0 - model.rect.x) / $Basics.toFloat(model.rect.w));
               } else {
                  return $Result.Err(_p3._0);
               }
         } else {
            var _p4 = A2($Json$Decode.decodeValue,getY,v);
            if (_p4.ctor === "Ok") {
                  return $Result.Ok($Basics.toFloat(_p4._0 - model.rect.y) / $Basics.toFloat(model.rect.h));
               } else {
   ...
   var SvgMoved = function (a) {
      return {ctor: "SvgMoved",_0: a};
   };
   var onMouseMove = A2(sliderEvt,"mousemove",SvgMoved);
   var Reply = function (a) {    return {ctor: "Reply",_0: a};};
   var UselessCrap = {ctor: "UselessCrap"};
   var updsend = F3(function (model,ut,loc) {
      var prest = !_U.eq(ut,Unpress);
      var bLoc = _U.cmp(loc,1.0) > 0 ? 1.0 : _U.cmp(loc,
      0.0) < 0 ? 0.0 : loc;
      if (_U.eq(model.location,bLoc) && _U.eq(model.pressed,prest))
      return {ctor: "_Tuple2",_0: model,_1: $Effects.none}; else {
            var um = A2($Json$Encode.encode,
            0,
            encodeUpdateMessage(A3(UpdateMessage,model.cid,ut,bLoc)));
            return {ctor: "_Tuple2"
                   ,_0: _U.update(model,{location: bLoc,pressed: prest})
                   ,_1: $Effects.task(A2($Task.andThen,
                   model.sendf(um),
                   function (_p5) {
                      return $Task.succeed(UselessCrap);
   ...
   var update = F2(function (action,model) {
      var _p6 = action;
      switch (_p6.ctor)
      {case "SvgPress": var _p7 = A2(getLocation,model,_p6._0);
           if (_p7.ctor === "Ok") {
                 return A3(updsend,model,Press,_p7._0);
              } else {
                 return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
              }
         case "SvgUnpress": var _p8 = model.pressed;
           if (_p8 === true) {
                 return A3(updsend,model,Unpress,model.location);
              } else {
                 return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
              }
         case "UselessCrap": return {ctor: "_Tuple2"
                                    ...
                                    ,_1: $Effects.none};
         case "Reply": return {ctor: "_Tuple2"
                              ,_0: _U.update(model,{name: _p6._0})
                              ,_1: $Effects.none};
         case "SvgMoved": var _p9 = model.pressed;
           if (_p9 === true) {
                 var _p10 = A2(getLocation,model,_p6._0);
                 if (_p10.ctor === "Ok") {
                       return A3(updsend,model,Move,_p10._0);
                    } else {
                       return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
                    }
              } else {
         ...
         case "SvgUpdate": var _p12 = _p6._0;
           var mod = function () {
              var _p11 = _p12.updateType;
              switch (_p11.ctor)
              {case "Press": return _U.update(model,
                   {pressed: true,location: _p12.location});
                 case "Move": return _U.update(model,{location: _p12.location});
                 default: return _U.update(model,
                   {pressed: false,location: _p12.location});}
           }();
           return {ctor: "_Tuple2",_0: mod,_1: $Effects.none};
         default: var _p14 = _p6._0;
           if ($List.isEmpty(_p14)) return model.pressed ? A3(updsend,
              model,
              Unpress,
              model.location) : {ctor: "_Tuple2",_0: model,_1: $Effects.none};
           else {
                 var _p13 = model.orientation;
                 if (_p13.ctor === "Horizontal") {
                       var locsum = A3($List.foldl,
                       F2(function (x,y) {    return x + y;}),
                       0,
                       A2($List.map,function (t) {    return t.x;},_p14));
                       var locavg = $Basics.toFloat(locsum) / $Basics.toFloat($List.length(_p14));
                       var loc = (locavg - $Basics.toFloat(model.rect.x)) / $Basics.toFloat(model.rect.w);
                       return model.pressed ? A3(updsend,model,Press,loc) : A3(updsend,
                       model,
                       Move,
                       loc);
                    } else {
                       ...
                       F2(function (x,y) {    return x + y;}),
                       0,
                       A2($List.map,function (t) {    return t.y;},_p14));
                       var locavg = $Basics.toFloat(locsum) / $Basics.toFloat($List.length(_p14));
                       var loc = (locavg - $Basics.toFloat(model.rect.y)) / $Basics.toFloat(model.rect.h);
                       return model.pressed ? A3(updsend,model,Press,loc) : A3(updsend,
                       model,
                       Move,
                       loc);
   ...
   var SvgUnpress = function (a) {
      return {ctor: "SvgUnpress",_0: a};
   };
   var onMouseLeave = A2(sliderEvt,"mouseleave",SvgUnpress);
   var onMouseUp = A2(sliderEvt,"mouseup",SvgUnpress);
   var SvgPress = function (a) {
      return {ctor: "SvgPress",_0: a};
   };
   var onMouseDown = A2(sliderEvt,"mousedown",SvgPress);
   var buttColor = function (pressed) {
      var _p15 = pressed;
      if (_p15 === true) {
            return "#f000f0";
   ...
   var view = F2(function (address,model) {
      var _p16 = function () {
         var _p17 = model.orientation;
         if (_p17.ctor === "Vertical") {
               return {ctor: "_Tuple4"
                      ,_0: model.srect.x
                      ,_1: $Basics.toString($Basics.round(model.location * $Basics.toFloat(model.rect.h)) + model.rect.y)
                      ,_2: model.srect.w
                      ,_3: "3"};
            } else {
               return {ctor: "_Tuple4"
                      ,_0: $Basics.toString($Basics.round(model.location * $Basics.toFloat(model.rect.w)) + model.rect.x)
                      ,_1: model.srect.y
                      ,_2: "3"
                      ,_3: model.srect.h};
            }
      ...
      _U.list([A2($Svg.rect,
              _U.list([$Svg$Attributes.x(model.srect.x)
                      ...
                      ,$Svg$Attributes.rx("2")
                      ,$Svg$Attributes.ry("2")
                      ,$Svg$Attributes.style("fill: #F1F1F1;")]),
              _U.list([]))
              ,A2($Svg.rect,
              _U.list([$Svg$Attributes.x(sx)
                      ,$Svg$Attributes.y(sy)
                      ,$Svg$Attributes.width(sw)
                      ,$Svg$Attributes.height(sh)
                      ,$Svg$Attributes.rx("2")
                      ,$Svg$Attributes.ry("2")
                      ,$Svg$Attributes.style(A2($Basics._op["++"],
                      "fill: ",
   ...
   A2($Json$Decode._op[":="],"name",$Json$Decode.string),
   A2($Json$Decode.andThen,
   A2($Json$Decode._op[":="],"orientation",$Json$Decode.string),
   $SvgThings.jsOrientation));
   return _elm.SvgSlider.values = {_op: _op
                                  ,Spec: Spec
                                  ,jsSpec: jsSpec
                                  ,Model: Model
                                  ,init: init
                                  ...
                                  ,onMouseUp: onMouseUp
                                  ,view: view};
};Elm.SvgLabel = Elm.SvgLabel || {};
Elm.SvgLabel.make = function (_elm) {
   "use strict";
   _elm.SvgLabel = _elm.SvgLabel || {};
   if (_elm.SvgLabel.values) return _elm.SvgLabel.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([$Svg$Attributes.x(model.srect.x)
              ...
              ,$Svg$Attributes.rx("15")
              ,$Svg$Attributes.ry("15")
              ,$Svg$Attributes.style("fill: #A1A1A1;")]),
      _U.list([]));
      var svgl = A2($List._op["::"],lbrect,model.textSvg);
      return A2($Svg.g,_U.list([]),svgl);
   });
   _op["=>"] = F2(function (v0,v1) {
      return {ctor: "_Tuple2",_0: v0,_1: v1};
   });
   ...
   var update = F2(function (action,model) {
      ...
      if (_p0.ctor === "SvgUpdate") {
            ...
            model.rect);
            return {ctor: "_Tuple2"
                   ,_0: _U.update(model,{label: _p1.label,textSvg: ts})
                   ,_1: $Effects.none};
         } else {
            return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         }
   ...
   var init = F3(function (rect,cid,spec) {
      var ts = A3($SvgThings.calcTextSvg,
      $SvgThings.ff,
      spec.label,
      rect);
   ...
   A2($Json$Decode._op[":="],"name",$Json$Decode.string),
   A2($Json$Decode._op[":="],"label",$Json$Decode.string));
   return _elm.SvgLabel.values = {_op: _op
                                 ,Spec: Spec
                                 ,jsSpec: jsSpec
                                 ,Model: Model
                                 ,init: init
                                 ...
                                 ,resize: resize
                                 ,view: view};
};Elm.SvgControl = Elm.SvgControl || {};
Elm.SvgControl.make = function (_elm) {
   "use strict";
   _elm.SvgControl = _elm.SvgControl || {};
   if (_elm.SvgControl.values) return _elm.SvgControl.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm),
   $Svg = Elm.Svg.make(_elm),
   $SvgButton = Elm.SvgButton.make(_elm),
   $SvgLabel = Elm.SvgLabel.make(_elm),
   $SvgSlider = Elm.SvgSlider.make(_elm),
   $SvgThings = Elm.SvgThings.make(_elm),
   $Task = Elm.Task.make(_elm),
   $Touch = Elm.Touch.make(_elm);
   var _op = {};
   ...
   var mkRlist = F4(function (orientation,
   rect,
   count,
   mbproportions) {
      var _p0 = orientation;
      if (_p0.ctor === "Horizontal") {
            ...
            if (_p1.ctor === "Nothing") {
                  return A2($SvgThings.hrects,rect,count);
               } else {
                  return A3($SvgThings.hrectsp,rect,count,_p1._0);
               }
         } else {
            var _p2 = mbproportions;
            if (_p2.ctor === "Nothing") {
                  return A2($SvgThings.vrects,rect,count);
               } else {
                  return A3($SvgThings.vrectsp,rect,count,_p2._0);
               }
         }
   });
   ...
   var SzCAction = F2(function (a,b) {
      return {ctor: "SzCAction",_0: a,_1: b};
   });
   var firstJust = F2(function (f,xs) {
      var _p3 = $List.head(xs);
      if (_p3.ctor === "Nothing") {
            return $Maybe.Nothing;
         } else {
            var _p4 = f(_p3._0);
            if (_p4.ctor === "Just") {
                  ...
                  return A2($Maybe.andThen,$List.tail(xs),firstJust(f));
               }
         }
   });
   var SzModel = F5(function (a,b,c,d,e) {
      return {cid: a
             ,rect: b
             ,controls: c
             ,orientation: d
   ...
   var processProps = function (lst) {
      var s = $List.sum(lst);
      return A2($List.map,function (x) {    return x / s;},lst);
   };
   var SzSpec = F3(function (a,b,c) {
      return {orientation: a,proportions: b,controls: c};
   });
   var myTail = function (lst) {
      var tl = $List.tail(lst);
      var _p5 = tl;
      if (_p5.ctor === "Just") {
            return _p5._0;
   ...
   var tupMap2 = F3(function (fa,fb,ab) {
      return {ctor: "_Tuple2"
             ,_0: fa($Basics.fst(ab))
             ,_1: fb($Basics.snd(ab))};
   });
   var controlName = function (mod) {
      var _p6 = mod;
      switch (_p6.ctor)
      {case "CmButton": return $Maybe.Just(_p6._0.name);
         case "CmSlider": return $Maybe.Just(_p6._0.name);
         case "CmLabel": return $Maybe.Just(_p6._0.name);
         default: return $Maybe.Nothing;}
   };
   var controlId = function (mod) {
      var _p7 = mod;
      switch (_p7.ctor)
      {case "CmButton": return _p7._0.cid;
         case "CmSlider": return _p7._0.cid;
         case "CmLabel": return _p7._0.cid;
         default: return _p7._0.cid;}
   };
   var findControl = F3(function (x,y,mod) {
      var _p8 = mod;
      switch (_p8.ctor)
      {case "CmButton": return A3($SvgThings.containsXY,
           _p8._0.rect,
           x,
           y) ? $Maybe.Just(mod) : $Maybe.Nothing;
         case "CmSlider": return A3($SvgThings.containsXY,
           _p8._0.rect,
           x,
           y) ? $Maybe.Just(mod) : $Maybe.Nothing;
         case "CmLabel": return $Maybe.Nothing;
         default: return A3(szFindControl,_p8._0,x,y);}
   });
   var szFindControl = F3(function (mod,x,y) {
      return A3($SvgThings.containsXY,mod.rect,x,y) ? A2(firstJust,
      A2(findControl,x,y),
      $Dict.values(mod.controls)) : $Maybe.Nothing;
   });
   var CaSizer = function (a) {
      return {ctor: "CaSizer",_0: a};
   };
   var toCtrlAction = F2(function (id,action) {
      var _p9 = $List.head(id);
      if (_p9.ctor === "Nothing") {
            return action;
         } else {
            return CaSizer(A2(SzCAction,
            _p9._0,
            A2(toCtrlAction,myTail(id),action)));
         }
   });
   var CaLabel = function (a) {
      return {ctor: "CaLabel",_0: a};
   };
   var CaSlider = function (a) {
      return {ctor: "CaSlider",_0: a};
   };
   var CaButton = function (a) {
      return {ctor: "CaButton",_0: a};
   };
   var controlTouchActionMaker = function (ctrl) {
      var _p10 = ctrl;
      switch (_p10.ctor)
      {case "CmButton": return function (t) {
              return $Maybe.Just(CaButton($SvgButton.SvgTouch(t)));
           };
         case "CmSlider": return function (t) {
              return $Maybe.Just(CaSlider($SvgSlider.SvgTouch(t)));
           };
         case "CmLabel": return function (t) {
              return $Maybe.Nothing;
           };
   ...
   var jsUmType = function (wat) {
      var _p11 = wat;
      switch (_p11)
      {case "button": return A2($Json$Decode.andThen,
           $SvgButton.jsUpdateMessage,
           function (x) {
              return $Json$Decode.succeed(A2(toCtrlAction,
              x.controlId,
              CaButton($SvgButton.SvgUpdate(x))));
           });
         case "slider": return A2($Json$Decode.andThen,
           $SvgSlider.jsUpdateMessage,
           function (x) {
              return $Json$Decode.succeed(A2(toCtrlAction,
              x.controlId,
              CaSlider($SvgSlider.SvgUpdate(x))));
           });
         case "label": return A2($Json$Decode.andThen,
           $SvgLabel.jsUpdateMessage,
           function (x) {
              return $Json$Decode.succeed(A2(toCtrlAction,
              x.controlId,
              CaLabel($SvgLabel.SvgUpdate(x))));
           });
         default: return $Json$Decode.fail(A2($Basics._op["++"],
           "unknown update type",
           wat));}
   };
   var jsUpdateMessage = A2($Json$Decode.andThen,
   A2($Json$Decode._op[":="],"controlType",$Json$Decode.string),
   jsUmType);
   var view = F2(function (address,model) {
      var _p12 = model;
      switch (_p12.ctor)
      {case "CmButton": return A2($SvgButton.view,
           A2($Signal.forwardTo,address,CaButton),
           _p12._0);
         case "CmSlider": return A2($SvgSlider.view,
           A2($Signal.forwardTo,address,CaSlider),
           _p12._0);
         case "CmLabel": return A2($SvgLabel.view,
           A2($Signal.forwardTo,address,CaLabel),
           _p12._0);
         default: return A2(szview,
           A2($Signal.forwardTo,address,CaSizer),
           _p12._0);}
   });
   var szview = F2(function (address,model) {
      var controllst = $Dict.toList(model.controls);
      return A2($Svg.g,
      _U.list([]),
      A2($List.map,viewSvgControls(address),controllst));
   });
   var viewSvgControls = F2(function (address,_p13) {
      var _p14 = _p13;
      return A2(view,
      A2($Signal.forwardTo,address,SzCAction(_p14._0)),
      _p14._1);
   });
   var CmSizer = function (a) {
      return {ctor: "CmSizer",_0: a};
   };
   var CmLabel = function (a) {
      return {ctor: "CmLabel",_0: a};
   };
   var CmSlider = function (a) {
      return {ctor: "CmSlider",_0: a};
   };
   var CmButton = function (a) {
      return {ctor: "CmButton",_0: a};
   };
   var update = F2(function (action,model) {
      var _p15 = {ctor: "_Tuple2",_0: action,_1: model};
      _v14_4: do {
         if (_p15.ctor === "_Tuple2") {
               switch (_p15._0.ctor)
               {case "CaButton": if (_p15._1.ctor === "CmButton") {
                          var _p16 = A2($SvgButton.update,_p15._0._0,_p15._1._0);
                          var a = _p16._0;
                          var b = _p16._1;
                          return {ctor: "_Tuple2"
                                 ,_0: CmButton(a)
                                 ,_1: A2($Effects.map,CaButton,b)};
                       } else {
                          break _v14_4;
                       }
                  case "CaSlider": if (_p15._1.ctor === "CmSlider") {
                          var _p17 = A2($SvgSlider.update,_p15._0._0,_p15._1._0);
                          var a = _p17._0;
                          var b = _p17._1;
                          return {ctor: "_Tuple2"
                                 ,_0: CmSlider(a)
                                 ,_1: A2($Effects.map,CaSlider,b)};
                       } else {
                          break _v14_4;
                       }
                  case "CaLabel": if (_p15._1.ctor === "CmLabel") {
                          var _p18 = A2($SvgLabel.update,_p15._0._0,_p15._1._0);
                          var a = _p18._0;
                          var b = _p18._1;
                          return {ctor: "_Tuple2"
                                 ,_0: CmLabel(a)
                                 ,_1: A2($Effects.map,CaLabel,b)};
                       } else {
                          break _v14_4;
                       }
                  default: if (_p15._1.ctor === "CmSizer") {
                          var _p19 = A2(szupdate,_p15._0._0,_p15._1._0);
                          var a = _p19._0;
                          var b = _p19._1;
                          return {ctor: "_Tuple2"
                                 ,_0: CmSizer(a)
                                 ,_1: A2($Effects.map,CaSizer,b)};
                       } else {
                          break _v14_4;
                       }}
            } else {
   ...
   var szupdate = F2(function (action,model) {
      var _p20 = action;
      var _p22 = _p20._0;
      var bb = A2($Dict.get,_p22,model.controls);
      var _p21 = bb;
      if (_p21.ctor === "Just") {
            var wha = A2(update,_p20._1,_p21._0);
            var updcontrols = A3($Dict.insert,
            _p22,
            $Basics.fst(wha),
            model.controls);
            var newmod = _U.update(model,{controls: updcontrols});
            return {ctor: "_Tuple2"
                   ,_0: newmod
                   ,_1: A2($Effects.map,SzCAction(_p22),$Basics.snd(wha))};
         } else {
            return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         }
   });
   var CsSizer = function (a) {
      return {ctor: "CsSizer",_0: a};
   };
   var CsLabel = function (a) {
      return {ctor: "CsLabel",_0: a};
   };
   var CsSlider = function (a) {
      return {ctor: "CsSlider",_0: a};
   };
   var CsButton = function (a) {
   ...
   var jsCs = function (t) {
      ...
      {case "button": return A2($Json$Decode.andThen,
           $SvgButton.jsSpec,
           function (a) {
              return $Json$Decode.succeed(CsButton(a));
           });
         case "slider": return A2($Json$Decode.andThen,
           $SvgSlider.jsSpec,
           function (a) {
              return $Json$Decode.succeed(CsSlider(a));
           });
         case "label": return A2($Json$Decode.andThen,
           $SvgLabel.jsSpec,
           function (a) {
              return $Json$Decode.succeed(CsLabel(a));
           });
         case "sizer": return A2($Json$Decode.andThen,
           jsSzSpec,
           function (a) {
              return $Json$Decode.succeed(CsSizer(a));
           });
         default: return $Json$Decode.fail(A2($Basics._op["++"],
           "unkown type: ",
           t));}
   };
   var jsSzSpec = A4($Json$Decode.object3,
   SzSpec,
   A2($Json$Decode.andThen,
   A2($Json$Decode._op[":="],"orientation",$Json$Decode.string),
   $SvgThings.jsOrientation),
   ...
   function (x) {
      return $Json$Decode.succeed(A2($Maybe.map,processProps,x));
   }),
   A2($Json$Decode._op[":="],
   "controls",
   $Json$Decode.list(lazy(function (_p24) {    return jsSpec;}))));
   var jsSpec = A2($Json$Decode.andThen,
   ...
   var resize = F2(function (model,rect) {
      var _p25 = model;
      switch (_p25.ctor)
      {case "CmButton": return A3(tupMap2,
           CmButton,
           $Effects.map(CaButton),
           A2($SvgButton.resize,
           _p25._0,
           A2($SvgThings.shrinkRect,border,rect)));
         case "CmSlider": return A3(tupMap2,
           CmSlider,
           $Effects.map(CaSlider),
           A2($SvgSlider.resize,
           _p25._0,
           A2($SvgThings.shrinkRect,border,rect)));
         case "CmLabel": return A3(tupMap2,
           CmLabel,
           $Effects.map(CaLabel),
           A2($SvgLabel.resize,
           _p25._0,
           A2($SvgThings.shrinkRect,border,rect)));
         default: return A3(tupMap2,
           CmSizer,
           $Effects.map(CaSizer),
           A2(szresize,_p25._0,rect));}
   });
   var szresize = F2(function (model,rect) {
      var clist = $Dict.toList(model.controls);
      var rlist = A4(mkRlist,
      model.orientation,
      rect,
      $List.length(clist),
      model.proportions);
      var ctlsNeffs = A2($List.map,
      function (_p26) {
         var _p27 = _p26;
         return {ctor: "_Tuple2"
                ,_0: _p27._0._0
                ,_1: A2(resize,_p27._0._1,_p27._1)};
      },
      A2(zip,clist,rlist));
      var controls = A2($List.map,
      function (_p28) {
         var _p29 = _p28;
         return {ctor: "_Tuple2",_0: _p29._0,_1: _p29._1._0};
      },
      ctlsNeffs);
      var cdict = $Dict.fromList(controls);
      var effs = $Effects.batch(A2($List.map,
      function (_p30) {
         var _p31 = _p30;
         return A2($Effects.map,
         function (ef) {
            return A2(SzCAction,_p31._0,ef);
         },
         _p31._1._1);
      },
      ctlsNeffs));
      return {ctor: "_Tuple2"
             ,_0: _U.update(model,{rect: rect,controls: cdict})
             ,_1: effs};
   });
   var init = F4(function (sendf,rect,cid,spec) {
      var _p32 = spec;
      switch (_p32.ctor)
      {case "CsButton": var _p33 = A4($SvgButton.init,
           sendf,
           A2($SvgThings.shrinkRect,border,rect),
           cid,
           _p32._0);
           ...
           return {ctor: "_Tuple2"
                  ,_0: CmButton(a)
                  ,_1: A2($Effects.map,CaButton,b)};
         case "CsSlider": var _p34 = A4($SvgSlider.init,
           sendf,
           A2($SvgThings.shrinkRect,border,rect),
           cid,
           _p32._0);
           ...
           return {ctor: "_Tuple2"
                  ,_0: CmSlider(a)
                  ,_1: A2($Effects.map,CaSlider,b)};
         case "CsLabel": var _p35 = A3($SvgLabel.init,
           A2($SvgThings.shrinkRect,border,rect),
           cid,
           _p32._0);
           var a = _p35._0;
           ...
           return {ctor: "_Tuple2"
                  ,_0: CmLabel(a)
                  ,_1: A2($Effects.map,CaLabel,b)};
         default: var _p36 = A4(szinit,sendf,rect,cid,_p32._0);
           var a = _p36._0;
           var b = _p36._1;
           return {ctor: "_Tuple2"
                  ,_0: CmSizer(a)
   ...
   var szinit = F4(function (sendf,rect,cid,szspec) {
      var idxs = _U.range(0,$List.length(szspec.controls));
      var rlist = A4(mkRlist,
      szspec.orientation,
      rect,
      $List.length(szspec.controls),
      szspec.proportions);
      var blist = A2($List.map,
      function (_p37) {
         var _p38 = _p37;
         return A4(init,
         sendf,
         _p38._1,
         A2($Basics._op["++"],cid,_U.list([_p38._2])),
         _p38._0);
      },
      A4($List.map3,
      ...
      rlist,
      idxs));
      var controlz = A2(zip,idxs,A2($List.map,$Basics.fst,blist));
      var fx = $Effects.batch(A2($List.map,
      function (_p39) {
         var _p40 = _p39;
         return A2($Effects.map,SzCAction(_p40._0),_p40._1);
      },
      A2(zip,idxs,A2($List.map,$Basics.snd,blist))));
      return {ctor: "_Tuple2"
             ,_0: A5(SzModel,
             cid,
             rect,
             $Dict.fromList(controlz),
             szspec.orientation,
             szspec.proportions)
             ,_1: fx};
   });
   return _elm.SvgControl.values = {_op: _op
                                   ,border: border
                                   ,CsButton: CsButton
                                   ,CsSlider: CsSlider
                                   ,CsLabel: CsLabel
                                   ...
                                   ,tupMap2: tupMap2
                                   ,resize: resize
                                   ,controlTouchActionMaker: controlTouchActionMaker
                                   ,jsSpec: jsSpec
                                   ,jsCs: jsCs
                                   ,jsUpdateMessage: jsUpdateMessage
                                   ,jsUmType: jsUmType
                                   ,myTail: myTail
                                   ,toCtrlAction: toCtrlAction
                                   ,update: update
                                   ,init: init
                                   ,view: view
                                   ,SzSpec: SzSpec
                                   ...
                                   ,szinit: szinit
                                   ,szview: szview
                                   ,viewSvgControls: viewSvgControls};
};Elm.Svg = Elm.Svg || {};
...
Elm.SvgTouch = Elm.SvgTouch || {};
Elm.SvgTouch.make = function (_elm) {
   "use strict";
   _elm.SvgTouch = _elm.SvgTouch || {};
   if (_elm.SvgTouch.values) return _elm.SvgTouch.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$SvgTouch = Elm.Native.SvgTouch.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var taps = $Native$SvgTouch.taps;
   var touches = $Native$SvgTouch.touches;
   var Touch = F6(function (a,b,c,d,e,f) {
      return {x: a,y: b,id: c,x0: d,y0: e,t0: f};
   });
   return _elm.SvgTouch.values = {_op: _op
                                 ,touches: touches
                                 ,taps: taps
                                 ,Touch: Touch};
};Elm.SvgControlPage = Elm.SvgControlPage || {};
Elm.SvgControlPage.make = function (_elm) {
   "use strict";
   _elm.SvgControlPage = _elm.SvgControlPage || {};
   if (_elm.SvgControlPage.values)
   return _elm.SvgControlPage.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $Svg = Elm.Svg.make(_elm),
   $Svg$Attributes = Elm.Svg.Attributes.make(_elm),
   $SvgControl = Elm.SvgControl.make(_elm),
   $SvgThings = Elm.SvgThings.make(_elm),
   $SvgTouch = Elm.SvgTouch.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   _op["=>"] = F2(function (v0,v1) {
      return {ctor: "_Tuple2",_0: v0,_1: v1};
   });
   var updict = F2(function (mt,dict) {
      return A3($Dict.update,
      $SvgControl.controlId($Basics.fst(mt)),
      function (mbpair) {
         var _p0 = mbpair;
         if (_p0.ctor === "Nothing") {
               return $Maybe.Just({ctor: "_Tuple2"
                                  ,_0: $SvgControl.controlTouchActionMaker($Basics.fst(mt))
                                  ,_1: _U.list([$Basics.snd(mt)])});
            } else {
               return $Maybe.Just({ctor: "_Tuple2"
                                  ,_0: _p0._0._0
                                  ,_1: A2($List._op["::"],$Basics.snd(mt),_p0._0._1)});
            }
      },
      dict);
   });
   var touchDict = F2(function (root,touches) {
      var meh = A2($List.filterMap,
      function (t) {
         return A2($Maybe.andThen,
         A3($SvgControl.findControl,t.x,t.y,root),
         function (c) {
            return $Maybe.Just({ctor: "_Tuple2",_0: c,_1: t});
         });
      },
      touches);
      return A3($List.foldl,updict,$Dict.empty,meh);
   });
   var JmUpdate = function (a) {
      return {ctor: "JmUpdate",_0: a};
   };
   var JmSpec = function (a) {    return {ctor: "JmSpec",_0: a};};
   var Touche = function (a) {    return {ctor: "Touche",_0: a};};
   var WinDims = function (a) {
      return {ctor: "WinDims",_0: a};
   };
   var CAction = function (a) {
      return {ctor: "CAction",_0: a};
   };
   var viewSvgControl = F2(function (address,conmodel) {
      return A2($SvgControl.view,
      A2($Signal.forwardTo,address,CAction),
      conmodel);
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Svg.svg,
      _U.list([$Svg$Attributes.width(model.srect.w)
              ,$Svg$Attributes.height(model.srect.h)
              ...
              A2($Basics._op["++"],
              model.srect.w,
              A2($Basics._op["++"]," ",model.srect.h)))))))]),
      _U.list([A2(viewSvgControl,address,model.control)]))]));
   });
   var JsonMsg = function (a) {
   ...
   var init = F3(function (sendf,rect,spec) {
      var statefx = A2($List.map,
      function (t) {
         return $Effects.task($Task.succeed(CAction(t)));
      },
      A2($Maybe.withDefault,_U.list([]),spec.state));
      var _p1 = A4($SvgControl.init,
      sendf,
      rect,
      _U.list([]),
      spec.rootControl);
      var conmod = _p1._0;
      var conevt = _p1._1;
      var fx = $Effects.batch(A2($List._op["::"],
      A2($Effects.map,CAction,conevt),
      statefx));
      return {ctor: "_Tuple2"
             ,_0: A7(Model,
             spec.title,
   ...
   var Spec = F3(function (a,b,c) {
      return {title: a,rootControl: b,state: c};
   });
   var jsSpec = A4($Json$Decode.object3,
   Spec,
   A2($Json$Decode._op[":="],"title",$Json$Decode.string),
   A2($Json$Decode._op[":="],"rootControl",$SvgControl.jsSpec),
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "state",
   $Json$Decode.list($SvgControl.jsUpdateMessage))));
   var jsMessage = $Json$Decode.oneOf(_U.list([A2($Json$Decode.andThen,
                                              jsSpec,
                                              function (x) {
                                                 return $Json$Decode.succeed(JmSpec(x));
                                              })
                                              ,A2($Json$Decode.andThen,
                                              $SvgControl.jsUpdateMessage,
                                              function (x) {
                                                 return $Json$Decode.succeed(JmUpdate(CAction(x)));
                                              })]));
   var update = F2(function (action,model) {
      update: while (true) {
         var _p2 = action;
         switch (_p2.ctor)
         {case "JsonMsg": var _p3 = A2($Json$Decode.decodeString,
              jsMessage,
              _p2._0);
              if (_p3.ctor === "Ok") {
                    if (_p3._0.ctor === "JmSpec") {
                          return A3(init,model.mahsend,model.mahrect,_p3._0._0);
                       } else {
                          var _v3 = _p3._0._0,_v4 = model;
                          action = _v3;
                          model = _v4;
                          continue update;
                       }
                    ...
                    return {ctor: "_Tuple2"
                           ,_0: _U.update(model,{title: _p3._0})
                           ,_1: $Effects.none};
                 }
            case "CAction": var wha = A2($SvgControl.update,
              _p2._0,
              model.control);
              var newmod = _U.update(model,{control: $Basics.fst(wha)});
              return {ctor: "_Tuple2"
                     ,_0: newmod
                     ,_1: A2($Effects.map,CAction,$Basics.snd(wha))};
            case "WinDims": var nr = A4($SvgThings.Rect,
              0,
              0,
              _p2._0._0,
              _p2._0._1);
              var _p4 = A2($SvgControl.resize,model.control,nr);
              var ctrl = _p4._0;
              var eff = _p4._1;
              return {ctor: "_Tuple2"
                     ,_0: _U.update(model,
                     {mahrect: nr,srect: $SvgThings.toSRect(nr),control: ctrl})
                     ,_1: A2($Effects.map,CAction,eff)};
            default: var tdict = A2(touchDict,model.control,_p2._0);
              var curtouches = A2($Dict.map,
              F2(function (_p5,v) {    return $Basics.fst(v);}),
              tdict);
              var prevs = A2($Dict.diff,model.prevtouches,curtouches);
              return {ctor: "_Tuple2"
                     ,_0: _U.update(model,{prevtouches: curtouches})
                     ...
                     function (_p6) {
                        var _p7 = _p6;
                        return A2($Maybe.map,
                        $SvgControl.toCtrlAction(_p7._0),
                        _p7._1._0(_p7._1._1));
                     },
                     $Dict.toList(tdict))),
                     A2($List.map,
                     ...
                     function (_p8) {
                        var _p9 = _p8;
                        return A2($Maybe.map,
                        $SvgControl.toCtrlAction(_p9._0),
                        _p9._1(_U.list([])));
                     },
                     $Dict.toList(prevs)))))};}
      }
   });
   return _elm.SvgControlPage.values = {_op: _op
                                       ,Spec: Spec
                                       ,jsSpec: jsSpec
                                       ,Model: Model
                                       ,JsonMsg: JsonMsg
...
Elm.StartApp.make = function (_elm) {
   ...
   var start = function (config) {
      var updateStep = F2(function (_p1,_p0) {
         ...
         return {ctor: "_Tuple2"
                ,_0: newModel
                ,_1: $Effects.batch(_U.list([_p3._1,additionalEffects]))};
      });
      var update = F2(function (actions,_p5) {
         var _p6 = _p5;
         return A3($List.foldl,
         updateStep,
         {ctor: "_Tuple2",_0: _p6._0,_1: $Effects.none},
         actions);
      });
      var updateStart = function (actions) {
         return A3($List.foldl,
         updateStep,
         config.init,
         A2($List.filter,$Basics.fst,actions));
      };
      ...
      A2($Signal.map,
      $List.map(F2(function (v0,v1) {
         return {ctor: "_Tuple2",_0: v0,_1: v1};
      })(false)),
      messages.signal),
      A2($Basics._op["++"],
      A2($List.map,
      $Signal.map(function (_p7) {
         return singleton(A2(F2(function (v0,v1) {
            return {ctor: "_Tuple2",_0: v0,_1: v1};
         }),
         false,
         _p7));
      ...
      config.inits)));
      var effectsAndModel = A3($Signal$Extra.foldp$,
      update,
      updateStart,
      inputs);
      ...
      return {html: A2($Signal.map,config.view(address),model)
             ...
             ,tasks: A2($Signal.map,
             function (_p9) {
                return A2($Effects.toTask,messages.address,$Basics.snd(_p9));
             },
             effectsAndModel)};
   };
   ...
   var Config = F5(function (a,b,c,d,e) {
      return {init: a,update: b,view: c,inputs: d,inits: e};
   });
...
Elm.WebSocket.make = function (_elm) {
   ...
   if (_elm.WebSocket.values) return _elm.WebSocket.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var connected = $Native$WebSocket.connected;
   var send = $Native$WebSocket.send;
   var listen = $Native$WebSocket.listen;
   var create = $Native$WebSocket.create;
   var createToHost = $Native$WebSocket.createToHost;
   var WebSocket = {ctor: "WebSocket"};
   return _elm.WebSocket.values = {_op: _op
                                  ,WebSocket: WebSocket
                                  ,createToHost: createToHost
                                  ,create: create
                                  ,listen: listen
...
Elm.Main.make = function (_elm) {
   ...
   $SvgButton = Elm.SvgButton.make(_elm),
   $SvgControl = Elm.SvgControl.make(_elm),
   $SvgControlPage = Elm.SvgControlPage.make(_elm),
   $SvgThings = Elm.SvgThings.make(_elm),
   $SvgTouch = Elm.SvgTouch.make(_elm),
   $Task = Elm.Task.make(_elm),
   ...
   var _op = {};
   var inputKeyboard = A2($Signal.map,
   function (c) {
      return $Basics.toString(c);
   },
   $Keyboard.presses);
   var connected = $Signal.mailbox(false);
   var listen = $Signal.mailbox("");
   var socket = $WebSocket.createToHost("1234");
   var listening = Elm.Native.Task.make(_elm).perform(A2($Task.andThen,
   socket,
   function (s) {
      return $Task.sequence(_U.list([A2($WebSocket.listen,
                                    listen.address,
                                    s)
                                    ,A2($WebSocket.connected,connected.address,s)]));
   }));
   var send = function (message) {
      return A2($Task.andThen,socket,$WebSocket.send(message));
   };
   var sending = Elm.Native.Task.make(_elm).performSignal("sending",
   A2($Signal.map,send,inputKeyboard));
   var app = $StartApp.start({init: A3($SvgControlPage.init,
                             send,
                             A4($SvgThings.Rect,0,0,500,300),
                             A3($SvgControlPage.Spec,
                             "mehtitle",
                             $SvgControl.CsButton(A2($SvgButton.Spec,"blah",$Maybe.Nothing)),
                             $Maybe.Nothing))
                             ,update: $SvgControlPage.update
                             ,view: $SvgControlPage.view
                             ,inits: _U.list([A2($Signal.map,
                             $SvgControlPage.WinDims,
                             $Window.dimensions)])
                             ,inputs: _U.list([A2($Signal.map,
                                              $SvgControlPage.JsonMsg,
                                              listen.signal)
                                              ,A2($Signal.map,$SvgControlPage.WinDims,$Window.dimensions)
                                              ,A2($Signal.map,$SvgControlPage.Touche,$SvgTouch.touches)])});
   var main = app.html;
